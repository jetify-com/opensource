...
func newParameterStore(ctx context.Context, config *SSMConfig) (*parameterStore, error) {
	...
	client := ssm.NewFromConfig(awsConfig, func(o *ssm.Options) {
		...
		if (config.AccessKeyId != "" && config.SecretKey != "") || config.SessionToken != "" {
...
// returns parameter name and env name given a full parameter path
// e.g., input: /jetpack-data/env/org_id/proj_01234/dev/key123
