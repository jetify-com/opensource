...
func TestRecordAttrs(t *testing.T) {
	as := []Attr{
		Int("k1", 1), String("k2", "foo"), Int("k3", 3),
...
func TestRecordSource(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{0, "", "", false},
		{-16, "", "", false},
		{1, "go.jetify.com/pkg/serror/internal/record.TestRecordSource", "record_test.go", true}, // 1: caller of NewRecord
